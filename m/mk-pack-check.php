...
$result1zz=mysql_query("SELECT p_id, mk_profile FROM package WHERE status = '0' AND p_id = '$p_id'");
...
$mk_profile = $rows1['mk_profile'];
    ...
    if ($API->connect($ServerIP, $Username, $Pass, $Port)) {
		...
		if($remoteaddress != ''){ echo $remoteaddress; }
		else{ ?>
			<select name="" class="chzn-select" style="100%;color: red;height: 27px;font-weight: bold;" required="">
				<option value="">Bad Package !!</option>
			</select>
		<?php }
