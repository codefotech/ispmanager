...
$maczoneid = $_SESSION['SESS_MAC_ZID'];
if($userr_typ == 'mreseller'){
$sql88 = ("SELECT s.id, s.link, s.username, s.password, s.status, e.e_cont_per FROM sms_setup AS s LEFT JOIN emp_info AS e ON e.z_id = s.z_id WHERE s.status = '0' AND s.z_id = '$maczoneid'");
