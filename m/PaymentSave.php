...
if($payment_idz == $last_payment_idz){
	...
	$sqlmqq = mysql_query("SELECT p.id AS mrno, z.z_name, c.bandwidth, c.cell, c.ip, x.mk_profile, c.mac, c.z_id, c.c_name, p.bill_discount, e.e_name as pay_ent_byname, p.pay_ent_by, c.c_id, c.com_id FROM payment as p
		...
		$tttee = $dew -($payment + $billdiscount);
...
$action_details = 'Add Payment: '.$payment.' tk, Discount: '.$bill_disc.', MR-TRX No: '.$moneyreceiptno.$trxidd.', Bank: '.$bank.$onlinebank;
$query222 = "INSERT INTO emp_log (e_id, c_id, module_name, titel, date, time, action, action_details) VALUES ('$pay_ent_by', '$c_id', 'Billing', 'Payment', '$update_date', '$update_time', 'Add_payment', '$action_details')";
