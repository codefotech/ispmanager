...
$API->debug = false;
  ...
  if($_POST) 
  {
    ...
    if ($API->connect($ServerIP, $Username, $Pass, $Port)) {
		...
		$ppp_name = $res[0]['name']; 
		if($ppp_name == ''){?>
		<a style="font-weight: bold;color: green;font-weight: bold;">THIS IP IS FREE</a>
		<?php }else{ ?> 
			<select name="" class="chzn-select" style="width:100%;color: red;height: 27px;font-weight: bold;" required="">
