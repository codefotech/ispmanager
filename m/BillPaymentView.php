...
$result = mysql_query("SELECT c.c_id, c.com_id, c.c_name, c.cell, c.address, p.p_name, p.p_price FROM clients AS c
	...
	<div class="box box-primary">
		...
		<div class="navbar">
            <div class="navbar-inner">
				...
				<div class="deadlol">[<b>Company ID :</b><?php echo $row['com_id']; ?>]</div>
				...
				<table class="table table-bordered responsive" style="width: 100% !important;float: left;">
                    ...
                    <thead>
                        <tr  class="newThead">
							...
							<th class="head0" style="font-size: 9px;">Print</th>
                        </tr>
                    </thead>
                    <tbody>
						<?php		
								...
								{
									if($rown['pay_idd'] != '#'){
											$ee = "<form action='fpdf/MRPrint' method='post' enctype='multipart/form-data'><input type='hidden' name='mrno' value='{$rown['pay_idd']}'/><input type='hidden' name='c_id' value='{$rown['c_id']}'/><button class='btn ownbtn2'><i class='iconfa-print'></i></button></form>";
										} else{
											$ee = '';
										}
									...
									echo
										"<tr class='gradeX'>
											...
											<td style='font-size: 9px;font-weight: bold;'>{$ee}</td>
										</tr>\n ";
								}  
							?>
					</tbody>
