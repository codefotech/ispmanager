...
$result=mysql_query("SELECT z.z_id, z.z_name, e.mk_id, e.e_name AS resellername, e.e_id AS resellerid, e.minimum_day, e.minimum_days, e.minimum_sts, e.prefix FROM zone AS z LEFT JOIN emp_info AS e ON e.e_id = z.e_id WHERE z.status = '0' AND z.e_id = '$e_id'");
}
else{
$result=mysql_query("SELECT z.z_id, z.z_name, e.mk_id, e.e_name AS resellername, e.e_id AS resellerid, e.minimum_day, e.minimum_days, e.minimum_sts, e.prefix FROM zone AS z LEFT JOIN emp_info AS e ON e.e_id = z.e_id WHERE z.status = '0' AND z.e_id != '' AND z.z_id = '$z_id'");
}
		...
		<div class="box-header" style="padding: 0px 10px 0px 10px;">
					<form class="stdform" method="post" action="MACClientAddQuery" enctype="multipart/form-data">
						...
						<?php } ?>
							...
							<p>
								<label style="width: 100%;font-weight: bold;font-size: 10px;">PPPoE ID*</label>
								<p id="result" style="font-weight: bold;"></p>
								<input type="text" name="c_id" id="name" placeholder="User id must be at least 3 characters long" style="width:100%;" value = '<?php echo $row['prefix'];?>' class="input-xxlarge" required="" />
							</p>
							<p>
							...
							<p>
								...
								<div id="result133"></div>
								<span class="field" style="margin-left: 0px;"><input type="text" style="width:100%;" id="ip" name="ip" placeholder="Ex: 192.168.XX.XX" class="input-xlarge"/> <div id="result1"></div></span>
							</p>
							<p>
					...
					<div class="modal-footer">
						...
						<button class="btn ownbtn12" type="submit">Submit</button>
					</div>
			</form>	
		</div>
				 ...
				 {  
				  ...
				  {  
				   ...
				   $.ajax({
					...
					type : 'POST',
					url  : "ip-mac-check1.php?mk_id="+<?php echo $mkkk_id;?>,
					data : $(this).serialize(),
					success : function(data)
						{
							  $("#result133").html(data);
