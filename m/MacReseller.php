...
$sql = mysql_query("SELECT e.id, e.e_id, e.e_name, e.billing_type, e.minimum_day, m.Name AS mkname, e.pre_address, e.e_cont_per, e.e_j_date, z.z_id, z.z_name, COUNT(c.c_id) AS totalclient, l.totalbill AS totalbills, t.totalpayments AS totalpay, t.totaldiscount AS totaldiscounts, (IFNULL(t.totalpayment,0) - l.totalbill) AS totaldue, e.over_due FROM emp_info AS e
				...
				<table id="dyntable" class="table table-bordered responsive">
                    ...
                    <thead>
                        <tr  class="newThead">
							...
							<th class="head0" style="font-size: 10px;padding: 5px;text-align: center;">Limit | Balance</th>
                        </tr>
                    </thead>
                    <tbody>
						<?php
								...
								if($row['billing_type'] == 'Postpaid'){
									...
									$over_due_limit = '';
									$corrr2 = 'style="color:#555555; font-weight: bold;font-size: 12px;"';
								}
								else{
								...
								$user_iddds = $rdga1['user_id'];
								$passfgdfg = $rdga1['password'];
										...
										"<tr class='gradeX'>
											<td class='center'>
												<ul class='tooltipsample' style='margin-bottom: 0px !important;'>
													<li style='font-size: 10px;font-weight: bold;line-height: 17px;'>{$row['e_name']}<br/>{$row['e_id']}<br/>{$row['e_cont_per']}</li>
												</ul>
											</td>
											<td class='center'>
												<ul class='tooltipsample' style='margin-bottom: 0px !important;'>
											...
											<td class='center'>
												<ul class='tooltipsample'>
													<li><b $corrr2>{$over_due_limit}</b><b $color>{$billz}</b></li><br/>
													<li style='margin-right: 0px;'><a style='font-size: 10px;font-weight: bold;' href='MacResellerPayment?id={$row['z_id']}' data-original-title='Payment' class='btn ownbtn3'><i class='iconfa-money'></i></a></li>
													<li style='margin-right: 0px;'><form action='login_exec' method='post'><input type='hidden' name='location_service' value='0'/><input type='hidden' name='username' value='{$user_iddds}' /><input type='hidden' name='wayy' value='loginasuser' /><input type='hidden' name='passwordd' value='{$passfgdfg}' /><button class='btn ownbtn4' style='border-radius: 3px;font-size: 10px;' title='Login As Reseller'><i class='iconfa-signin'></i></button></form></li>
												</ul>
											</td>
										</tr>\n ";
								}
