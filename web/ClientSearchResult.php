  ...
  if($userr_typp != '') 
  {
     ...
     if($comid != ''){
			$sql2 = "SELECT c.c_name, c.com_id, e.e_name AS technician, c.c_id, c.payment_deadline, c.b_date, c.termination_date, c.onu_mac, m.Name, c.mac_user, c.breseller, c.p_id, p.p_name, p.bandwith, p.p_price, c.raw_download, c.raw_upload, c.youtube_bandwidth, c.total_bandwidth, c.bandwidth_price, c.youtube_price, c.total_price, c.address, z.z_name, c.cell, c.note, c.note_auto, c.con_sts, DATE_FORMAT(c.join_date, '%D %M %Y') AS join_date, c.p_id, l.log_sts, l.pw, l.image, l.nid_fond, l.nid_back FROM clients AS c
					...
					WHERE c.sts = '0' AND c.com_id = '$comid'";
					
		if($userr_typp == 'billing'){
			$sql2 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
			$sql2 .= " AND c.mac_user !='1'";
		}
			$sql2 .= " ORDER BY c.com_id ASC";
		
		$sql = mysql_query($sql2);
		

		$query26 = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND c.com_id = '$comid' AND c.con_sts = 'Active'";
		if($userr_typp == 'billing'){
			$query26 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
			$query26 .= " AND c.mac_user !='1'";
		}
			$query2266 = mysql_query($query26);
			
		$query26e = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND c.com_id = '$comid' AND c.con_sts = 'Inactive'";
		if($userr_typp == 'billing'){
			$query26e .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
			$query26e .= " AND c.mac_user !='1'";
		}
			$query66 = mysql_query($query26e);
	 ...
	 if($cid != ''){
		...
		$sql2 = "SELECT c.c_name, c.com_id, e.e_name AS technician, c.c_id, c.payment_deadline, c.b_date, c.termination_date, c.onu_mac, m.Name, c.mac_user, c.breseller, c.p_id, p.p_name, p.bandwith, p.p_price, c.raw_download, c.raw_upload, c.youtube_bandwidth, c.total_bandwidth, c.bandwidth_price, c.youtube_price, c.total_price, c.address, z.z_name, c.cell, c.note, c.note_auto, c.con_sts, DATE_FORMAT(c.join_date, '%D %M %Y') AS join_date, c.p_id, l.log_sts, l.pw, l.image, l.nid_fond, l.nid_back FROM clients AS c
					...
					LEFT JOIN mk_con AS m ON m.id = c.mk_id 
					LEFT JOIN emp_info AS e ON e.e_id = c.technician
					WHERE c.sts = '0' AND c.c_id LIKE '$ccid'";
		if($userr_typp == 'billing'){
			$sql2 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26 = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND c_id LIKE '$ccid' AND c.con_sts = 'Active'";
		if($userr_typp == 'billing'){
			$query26 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26e = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND c_id LIKE '$ccid' AND c.con_sts = 'Inactive'";
		if($userr_typp == 'billing'){
			$query26e .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
	  ...
	  if($cname != ''){
		...
		$sql2 = "SELECT c.c_name, c.com_id, e.e_name AS technician, c.c_id, c.payment_deadline, c.b_date, c.termination_date, c.onu_mac, m.Name, c.mac_user, c.breseller, c.p_id, p.p_name, p.bandwith, p.p_price, c.raw_download, c.raw_upload, c.youtube_bandwidth, c.total_bandwidth, c.bandwidth_price, c.youtube_price, c.total_price, c.address, z.z_name, c.cell, c.note, c.note_auto, c.con_sts, DATE_FORMAT(c.join_date, '%D %M %Y') AS join_date, c.p_id, l.log_sts, l.pw, l.image, l.nid_fond, l.nid_back FROM clients AS c
					...
					LEFT JOIN mk_con AS m ON m.id = c.mk_id 
					LEFT JOIN emp_info AS e ON e.e_id = c.technician
					WHERE c.sts = '0' AND c.c_name LIKE '$ccname'";
		if($userr_typp == 'billing'){
			$sql2 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26 = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND c_name LIKE '$ccname' AND c.con_sts = 'Active'";
		if($userr_typp == 'billing'){
			$query26 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26e = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND c_name LIKE '$ccname' AND c.con_sts = 'Inactive'";
		if($userr_typp == 'billing'){
			$query26e .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
	 ...
	 if($cell != ''){
		...
		$sql2 = "SELECT c.c_name, c.com_id, e.e_name AS technician, c.c_id, c.payment_deadline, c.b_date, c.termination_date, c.onu_mac, m.Name, c.mac_user, c.breseller, c.p_id, p.p_name, p.bandwith, p.p_price, c.raw_download, c.raw_upload, c.youtube_bandwidth, c.total_bandwidth, c.bandwidth_price, c.youtube_price, c.total_price, c.address, z.z_name, c.cell, c.note, c.note_auto, c.con_sts, DATE_FORMAT(c.join_date, '%D %M %Y') AS join_date, c.p_id, l.log_sts, l.pw, l.image, l.nid_fond, l.nid_back FROM clients AS c
						...
						LEFT JOIN mk_con AS m ON m.id = c.mk_id 
						LEFT JOIN emp_info AS e ON e.e_id = c.technician
						WHERE c.sts = '0' AND c.cell != '' AND c.cell LIKE '$phone'";
		if($userr_typp == 'billing'){
			$sql2 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26 = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND cell != '' AND cell LIKE '$phone' AND c.con_sts = 'Active'";
		if($userr_typp == 'billing'){
			$query26 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26e = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND cell != '' AND cell LIKE '$phone' AND c.con_sts = 'Inactive'";
		if($userr_typp == 'billing'){
			$query26e .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
	 ...
	 if($address != ''){
		...
		$sql2 = "SELECT c.c_name, c.com_id, e.e_name AS technician, c.c_id, c.payment_deadline, c.b_date, c.termination_date, c.onu_mac, m.Name, c.mac_user, c.breseller, c.p_id, p.p_name, p.bandwith, p.p_price, c.raw_download, c.raw_upload, c.youtube_bandwidth, c.total_bandwidth, c.bandwidth_price, c.youtube_price, c.total_price, c.address, z.z_name, c.cell, c.note, c.note_auto, c.con_sts, DATE_FORMAT(c.join_date, '%D %M %Y') AS join_date, c.p_id, l.log_sts, l.pw, l.image, l.nid_fond, l.nid_back FROM clients AS c
					...
					LEFT JOIN mk_con AS m ON m.id = c.mk_id 
					LEFT JOIN emp_info AS e ON e.e_id = c.technician
					WHERE c.sts = '0' AND c.address != '' AND c.address LIKE '$aaa'";
		...
		$query26 = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND address != '' AND address LIKE '$aaa' AND c.con_sts = 'Active'";
		if($userr_typp == 'billing'){
			$query26 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26e = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND address != '' AND address LIKE '$aaa' AND c.con_sts = 'Inactive'";
		if($userr_typp == 'billing'){
			$query26e .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
	 ...
	 if($onu_mac != ''){
		...
		$sql2 = "SELECT c.c_name, c.com_id, e.e_name AS technician, c.c_id, c.payment_deadline, c.b_date, c.termination_date, m.Name, c.onu_mac, c.mac_user, c.breseller, c.p_id, p.p_name, p.bandwith, p.p_price, c.raw_download, c.raw_upload, c.youtube_bandwidth, c.total_bandwidth, c.bandwidth_price, c.youtube_price, c.total_price, c.address, z.z_name, c.cell, c.note, c.note_auto, c.con_sts, DATE_FORMAT(c.join_date, '%D %M %Y') AS join_date, c.p_id, l.log_sts, l.pw, l.image, l.nid_fond, l.nid_back FROM clients AS c
					...
					LEFT JOIN mk_con AS m ON m.id = c.mk_id 
					LEFT JOIN emp_info AS e ON e.e_id = c.technician
					WHERE c.sts = '0' AND c.onu_mac != '' AND c.onu_mac LIKE '$bbb'";
		...
		$query26 = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND onu_mac != '' AND onu_mac LIKE '$bbb' AND c.con_sts = 'Active'";
		if($userr_typp == 'billing'){
			$query26 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26e = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND onu_mac != '' AND onu_mac LIKE '$bbb' AND c.con_sts = 'Inactive'";
		if($userr_typp == 'billing'){
			$query26e .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
	 ...
	 if($ip != ''){
		...
		$sql2 = "SELECT c.c_name, c.com_id, e.e_name AS technician, c.c_id, c.payment_deadline, c.b_date, c.termination_date, m.Name, c.onu_mac, c.mac_user, c.breseller, c.p_id, p.p_name, p.bandwith, p.p_price, c.raw_download, c.raw_upload, c.youtube_bandwidth, c.total_bandwidth, c.bandwidth_price, c.youtube_price, c.total_price, c.address, z.z_name, c.cell, c.note, c.note_auto, c.con_sts, DATE_FORMAT(c.join_date, '%D %M %Y') AS join_date, c.p_id, l.log_sts, l.pw, l.image, l.nid_fond, l.nid_back FROM clients AS c
					...
					LEFT JOIN mk_con AS m ON m.id = c.mk_id 
					LEFT JOIN emp_info AS e ON e.e_id = c.technician
					WHERE c.sts = '0' AND c.ip != '' AND c.ip LIKE '$bbb'";
		...
		$query26 = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND ip != '' AND ip LIKE '$bbb' AND c.con_sts = 'Active'";
		if($userr_typp == 'billing'){
			$query26 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26e = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND ip != '' AND ip LIKE '$bbb' AND c.con_sts = 'Inactive'";
		if($userr_typp == 'billing'){
			$query26e .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
	 ...
	 if($cable_type != ''){
		$sql2 = "SELECT c.c_name, c.com_id, e.e_name AS technician, c.c_id, c.payment_deadline, c.b_date, c.termination_date, c.onu_mac, m.Name, c.mac_user, c.breseller, c.p_id, p.p_name, p.bandwith, p.p_price, c.raw_download, c.raw_upload, c.youtube_bandwidth, c.total_bandwidth, c.bandwidth_price, c.youtube_price, c.total_price, c.address, z.z_name, c.cell, c.note, c.note_auto, c.con_sts, DATE_FORMAT(c.join_date, '%D %M %Y') AS join_date, c.p_id, l.log_sts, l.pw, l.image, l.nid_fond, l.nid_back FROM clients AS c
					...
					LEFT JOIN mk_con AS m ON m.id = c.mk_id 
					LEFT JOIN emp_info AS e ON e.e_id = c.technician
					WHERE c.sts = '0' AND c.cable_type != '' AND c.cable_type = '$cable_type'";
		if($userr_typp == 'billing'){
			$sql2 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26 = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND cable_type != '' AND cable_type = '$cable_type' AND c.con_sts = 'Active'";
		if($userr_typp == 'billing'){
			$query26 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26e = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND cable_type != '' AND cable_type = '$cable_type' AND c.con_sts = 'Inactive'";
		if($userr_typp == 'billing'){
			$query26e .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
	...
	if($z_id != ''){
		$sql2 = "SELECT c.c_name, c.com_id, e.e_name AS technician, c.c_id, c.payment_deadline, c.b_date, c.termination_date, c.onu_mac, m.Name, c.mac_user, c.breseller, c.p_id, p.p_name, p.bandwith, p.p_price, c.raw_download, c.raw_upload, c.youtube_bandwidth, c.total_bandwidth, c.bandwidth_price, c.youtube_price, c.total_price, c.address, z.z_name, c.cell, c.note, c.note_auto, c.con_sts, DATE_FORMAT(c.join_date, '%D %M %Y') AS join_date, c.p_id, l.log_sts, l.pw, l.image, l.nid_fond, l.nid_back FROM clients AS c
					...
					LEFT JOIN mk_con AS m ON m.id = c.mk_id 
					LEFT JOIN emp_info AS e ON e.e_id = c.technician
					WHERE c.sts = '0' AND c.z_id != '' AND c.z_id = '$z_id'";
		if($userr_typp == 'billing'){
			$sql2 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26 = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND z_id != '' AND z_id = '$z_id' AND c.con_sts = 'Active'";
		if($userr_typp == 'billing'){
			$query26 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26e = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND z_id != '' AND z_id = '$z_id' AND c.con_sts = 'Inactive'";
		if($userr_typp == 'billing'){
			$query26e .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
	 ...
	 if($payment_deadline != ''){
		...
		$sql2 = "SELECT c.c_name, c.com_id, e.e_name AS technician, c.c_id, c.payment_deadline, c.b_date, c.onu_mac, m.Name, c.mac_user, c.breseller, c.p_id, p.p_name, p.bandwith, p.p_price, c.raw_download, c.raw_upload, c.youtube_bandwidth, c.total_bandwidth, c.bandwidth_price, c.youtube_price, c.total_price, c.address, z.z_name, c.cell, c.note, c.note_auto, c.con_sts, DATE_FORMAT(c.join_date, '%D %M %Y') AS join_date, c.p_id, l.log_sts, l.pw, l.image, l.nid_fond, l.nid_back FROM clients AS c
					...
					LEFT JOIN mk_con AS m ON m.id = c.mk_id 
					LEFT JOIN emp_info AS e ON e.e_id = c.technician
					WHERE c.sts = '0' AND c.payment_deadline != '' AND c.payment_deadline = '$fhhjdj'";
		...
		$query26 = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND payment_deadline = '$fhhjdj' AND c.con_sts = 'Active'";
		if($userr_typp == 'billing'){
			$query26 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26e = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND payment_deadline = '$fhhjdj' AND c.con_sts = 'Inactive'";
		if($userr_typp == 'billing'){
			$query26e .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
	 ...
	 if($b_date != ''){
		...
		$sql2 = "SELECT c.c_name, c.com_id, e.e_name AS technician, c.c_id, c.payment_deadline, c.b_date, c.onu_mac, m.Name, c.mac_user, c.breseller, c.p_id, p.p_name, p.bandwith, p.p_price, c.raw_download, c.raw_upload, c.youtube_bandwidth, c.total_bandwidth, c.bandwidth_price, c.youtube_price, c.total_price, c.address, z.z_name, c.cell, c.note, c.note_auto, c.con_sts, DATE_FORMAT(c.join_date, '%D %M %Y') AS join_date, c.p_id, l.log_sts, l.pw, l.image, l.nid_fond, l.nid_back FROM clients AS c
					...
					LEFT JOIN mk_con AS m ON m.id = c.mk_id 
					LEFT JOIN emp_info AS e ON e.e_id = c.technician
					WHERE c.sts = '0' AND c.b_date != '' AND c.b_date = '$fhhjdjd'";
		...
		$query26 = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND b_date = '$fhhjdjd' AND c.con_sts = 'Active'";
		if($userr_typp == 'billing'){
			$query26 .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
		...
		$query26e = "SELECT count(id) as active_count FROM clients AS c LEFT JOIN zone AS z ON z.z_id = c.z_id WHERE c.sts = '0' AND b_date = '$fhhjdjd' AND c.con_sts = 'Inactive'";
		if($userr_typp == 'billing'){
			$query26e .= " AND FIND_IN_SET('$e_id', z.emp_id) > 0";
		}
		if($search_with_reseller_client != '1'){
