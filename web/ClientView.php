...
$sqlmk1 = mysql_query("SELECT id, ServerIP, Username, Pass, Port, e_Md, online_sts, tx_rx_count_sts, secret_h, graph, web_port FROM mk_con WHERE sts = '0' AND id = '$mk_id'");
		...
		$tx_rx_count_sts = $rowmk1['tx_rx_count_sts'];
...
$drhdrhdd = mysql_query("SELECT (sum(tx_byte)/1000000000) AS tx_byte, (sum(rx_byte)/1000000000) AS rx_byte FROM client_bandwidth WHERE c_id = '$c_id' AND MONTH(date_time) = MONTH('$dateTime_rightnow') AND YEAR(date_time) = YEAR('$dateTime_rightnow')");
							...
							<div class="row" style="margin-top: 10px;border-top: 1px solid #ddd;padding-top: 10px;">
								<div style="padding-left: 15px; width: 100%;">
									<table class="table table-bordered table-invoice" style="width: 47%; float: left;margin-right: 3%;">
										...
										<?php } else{ } if($tx_rx_count_sts == '1'){ ?>
										<tr>
											<td style="text-align: right;font-weight: bold;">Consumed On (<?php echo date("M");?>)</td>
											<td style="font-size: 16px;font-family: Helvetica Neue,Helvetica,Arial,sans-serif;"><b style="font-weight: bold;color:red;">D: <?php echo $month_tx;?>GB</b> | <b style="font-weight: bold;color:green;">U: <?php echo $month_rx;?>GB</b></td>
										</tr>
										<?php } ?>
									</table>
