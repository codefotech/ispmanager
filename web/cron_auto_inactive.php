...
$accpt_amount= $rowsm['accpt_amount'];
			...
			$sql = mysql_query("SELECT t.c_id, c.c_name, c.com_id, c.cell, (t.totalbillamount - IFNULL(l.totalpaymentamount, 0.00)) AS totdueclients, c.payment_deadline, c.mk_id, c.breseller, c.ip, c.mac, c.raw_download, c.raw_upload FROM
								...
								WHERE t.totalbillamount - IFNULL(l.totalpaymentamount, 0.00) >= '$accpt_amount' AND c.payment_deadline != '' AND c.mac_user = '0' AND c.con_sts = 'Active' AND c.payment_deadline = '$deadline'");
								
	while( $row = mysql_fetch_assoc($sql) ){
		
//	$from_page = 'Auto Inactive';
	$mkk_id = $row['mk_id'];
	$c_idd = $row['c_id'];
	$com_id = $row['com_id'];
	$c_name = $row['c_name'];
...
//	$cell = (int) filter_var($celltt, FILTER_SANITIZE_NUMBER_INT);
