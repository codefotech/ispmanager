...
$wayyy=$_GET['wayyy'];
$payfrom = isset($_GET['payfrom']) ? $_GET['payfrom'] : '';
$payfrom_cid = isset($_GET['e_id']) ? $_GET['e_id'] : '';
extract($_POST);
...
//---------- bKash -----------
if($gateway == 'bKash' && in_array(1, $online_getway) || $gateway == 'bKashT' && in_array(6, $online_getway)){
...
$payfrom = isset($_POST['payfrom']) ? $_POST['payfrom'] : '';
$payfrom_cid = $_POST['payerReferenceee'];
$statusCode=$_POST['statusCode'];
$statusMessage=$_POST['statusMessage'];
$paymentID=$_POST['paymentID'];
$agreementID=$_POST['agreementID'];
$payerReference=$_POST['payerReferenceee'];
$customerMsisdn=$_POST['customerMsisdn'];
$trxID=$_POST['trxID'];
$amount=$_POST['amount'];
$transactionStatus=$_POST['transactionStatus'];
$paymentExecuteTime=$_POST['paymentExecuteTime'];
$merchantInvoiceNumber=$_POST['merchantInvoiceNumber'];
$currency=$_POST['currency'];
$intent=$_POST['intent'];
$dewamount=$_POST['dewamount'];
}
...
if($payfrom == ''){
	...
	<h2>Please Wait. Payment Processing....</h2>
		 <form action="PaymentOnlineMacSuccess?gateway=<?php echo $gateway;?>" method="post" name="ok">
			...
			<input type="hidden" name="statusMessage" value="<?php echo $statusMessage;?>">
			<input type="hidden" name="paymentID" value="<?php echo $paymentID;?>">
			...
			<input type="hidden" name="customerMsisdn" value="<?php echo $customerMsisdn;?>">
			<input type="hidden" name="trxID" value="<?php echo $trxID;?>">
			<input type="hidden" name="amount" value="<?php echo $amount;?>">
			<input type="hidden" name="transactionStatus" value="<?php echo $transactionStatus;?>">
			<input type="hidden" name="paymentExecuteTime" value="<?php echo $paymentExecuteTime;?>">
			<input type="hidden" name="merchantInvoiceNumber" value="<?php echo $merchantInvoiceNumber;?>">
			<input type="hidden" name="currency" value="<?php echo $currency;?>">
			<input type="hidden" name="intent" value="<?php echo $intent;?>">
			<?php } ?>
...
$client_id = $e_id;
$query222 =mysql_query("SELECT c_id, c_name, cell, address, con_sts, b_date, payment_deadline, breseller, agent_id, count_commission, com_percent, mac_user, z_id FROM clients WHERE c_id = '$client_id'");
	$row22 = mysql_fetch_assoc($query222);
...
if($row22['mac_user'] == '0'){
	$sqlqqmm = mysql_query("SELECT COUNT(id) AS smsgo FROM sms_setup WHERE username != '' AND z_id = '' ");
	...
	$gopage = 'PaymentOnlineSuccess';
}
else{
	$sqlqqmm = mysql_query("SELECT COUNT(id) AS smsgo FROM sms_setup WHERE z_id = '$z_id' ");
	$row22m = mysql_fetch_assoc($sqlqqmm);
	...
	$gopage = 'PaymentOnlineMacClientSuccess';
}
	$ClientsBill = 'active';
	//---------- Permission -----------
	$user_type = $_SESSION['SESS_USER_TYPE'];
	...
	<h2>Payment Processing....</h2>
		 <form action="<?php echo $gopage;?>?gateway=<?php echo $gateway;?>&wayyy=<?php echo $wayyy;?>" method="post" name="ok">
					<input type="hidden" value="<?php echo $row22['c_id'];?>" name="pay_ent_by" />
					<input type="hidden" value="<?php echo date('Y-m-d', time());?>" name="pay_ent_date" />
					<input type="hidden" name="pay_amount" value="<?php echo $dewamount;?>" />
					<input type="hidden" name="wayyy" value="<?php echo $wayyy;?>" />
					<input type="hidden" name="sentsms" value="<?php echo $smsgo;?>" />
					<input type="hidden" name="con_sts" value="<?php echo $row22['con_sts'];?>" />
					<input type="hidden" name="mac_user" value="<?php echo $row22['mac_user'];?>" />
					<?php if($agent_id != ''){?>
					<input type="hidden" name="agent_id" value="<?php echo $agent_id;?>" />
					<input type="hidden" name="commission_sts" value="<?php echo $count_commission;?>" />
					<input type="hidden" name="com_percent" value="<?php echo $percent_count;?>" />
					<input type="hidden" name="commission_amount" value="<?php echo $comission;?>" />
					<?php } if($row22['con_sts'] == 'Inactive' && $row22['mac_user'] == '0'){ ?>
...
<h2>Payment Processing....</h2>
     <form action="<?php echo $gopage;?>?gateway=<?php echo $gateway;?>&wayyy=<?php echo $wayyy;?>" method="post" name="ok">
				...
				<input type="hidden" name="pay_amount" value="<?php echo $dewamount;?>" />
				<input type="hidden" name="wayyy" value="<?php echo $wayyy;?>" />
				<input type="hidden" name="payfrom" value="<?php echo $payfrom;?>" />
				<input type="hidden" name="sentsms" value="<?php echo $smsgo;?>" />
				<input type="hidden" name="con_sts" value="<?php echo $row22['con_sts'];?>" />
				<input type="hidden" name="mac_user" value="<?php echo $row22['mac_user'];?>" />
				<?php if($agent_id != ''){?>
...
if($wayyy == 'reseller'){
	$successCallbackUrll = $weblink.'PaymentOnlineQuery2?gateway='.$gateway.'&referenceId='.$invoice.$dewamount.'&wayyy=reseller&reseller_id='.$reseller_id;
	$failureCallbackUrll = $weblink.'PaymentOnline?gateway='.$gateway.'&sts=faild&wayyy=reseller';
	$cancelCallbackUrll = $weblink.'PaymentOnline?gateway='.$gateway.'&sts=canceled&wayyy=reseller';
}else{
	if($payfrom == 'external'){
		$successCallbackUrll = $weblink.'PaymentOnlineQuery2.php?gateway='.$gateway.'&referenceId='.$invoice.$dewamount.'&wayyy=client&payfrom=external&client_id='.$c_id;
		$failureCallbackUrll = $weblink.'PaymentOnlineExternal?clientid='.$c_id.'&gateway=iPay&sts=faild';
		$cancelCallbackUrll = $weblink.'PaymentOnlineExternal?clientid='.$c_id.'&gateway=iPay&sts=canceled';
	}
	else{
		$successCallbackUrll = $weblink.'PaymentOnlineQuery2.php?gateway='.$gateway.'&referenceId='.$invoice.$dewamount.'&wayyy=client&client_id='.$c_id;
		$failureCallbackUrll = $weblink.'PaymentOnline?gateway='.$gateway.'&sts=faild&wayyy=client';
		$cancelCallbackUrll = $weblink.'PaymentOnline?gateway='.$gateway.'&sts=canceled&wayyy=client';
	}
}
	$post_token=array(
...
if($wayyy == 'reseller'){
	$post_data['success_url'] = $weblink.'PaymentOnlineQuery2?gateway='.$gateway.'&wayyy=reseller&reseller_id='.$reseller_id;
	$post_data['fail_url'] = $weblink.'PaymentOnline?gateway='.$gateway.'&sts=faild&dewamount='.$dewamount.'&wayyy=reseller';
	$post_data['cancel_url'] = $weblink.'PaymentOnline?gateway='.$gateway.'&sts=canceled&dewamount='.$dewamount.'&wayyy=reseller';
	$post_data['cus_name'] = $reseller_id;
	$post_data['value_b'] = $reseller_id;
}
else{
	if($payfrom == 'external'){
		$post_data['success_url'] = $weblink.'PaymentOnlineQuery2?gateway='.$gateway.'&wayyy=client&payfrom=external&client_id='.$c_id;
		$post_data['fail_url'] = $weblink.'PaymentOnlineExternal?gateway='.$gateway.'&sts=faild';
		$post_data['cancel_url'] = $weblink.'PaymentOnlineExternal?gateway='.$gateway.'&sts=canceled';
		$post_data['cus_name'] = $c_id;
		$post_data['value_b'] = $c_id;
	}
	else{
		$post_data['success_url'] = $weblink.'PaymentOnlineQuery2?gateway='.$gateway.'&wayyy=client&client_id='.$c_id;
		$post_data['fail_url'] = $weblink.'PaymentOnline?gateway='.$gateway.'&sts=faild&wayyy=client';
		$post_data['cancel_url'] = $weblink.'PaymentOnline?gateway='.$gateway.'&sts=canceled&wayyy=client';
		$post_data['cus_name'] = $c_id;
		$post_data['value_b'] = $c_id;
	}
}
