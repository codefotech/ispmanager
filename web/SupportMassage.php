...
?>
	...
	<div class="box box-primary">
			<div class="box-body">
				<div class="messageview" style="border-bottom: medium none;min-height: 400px;">
				<?php if ($sts == '1') {?>
					<span class="wsasw">Closed by <?php echo $close_by; ?></span>
				<?php } else { if ($user_type == 'admin' || $user_type == 'superadmin' || $user_type == 'accounts' || $user_type == 'support') { if($ticket_sts == 'Solved') {?>
				...
				<?php }
							...
							<?php if ($user_type == 'admin' || $user_type == 'superadmin' || $user_type == 'accounts' || $user_type == 'support' || $user_type == 'billing')  { if($ticket_sts == 'Solved') {?>
							...
							<?php } if ($sts == '0'){if ($user_type == 'admin' || $user_type == 'superadmin' || $user_type == 'billing' || $user_type == 'support' || $user_type == 'accounts') {?>
							...
							<div class="btn-group pull-right">
								...
								<input type='hidden' name='back' value = '?id=<?php echo $id; ?>'/>
									<select name="assign" class="chzn-select" style="width:180px;">	
											<option>Assign Person</option>								
											<?php
											while ($row2=mysql_fetch_array($queryr)) { ?>	
												<option value="<?php echo $row2['e_id']?>"<?php if($assign == $row2['e_id']) echo 'selected="selected"';?>><?php echo $row2['e_name'];?> || <?php echo $row2['e_id'];?></option>		
											<?php } ?>
									</select>
									<button class="btn col1" style="float: right;padding: 9.5px 10px;" type="submit"><i class='iconfa-ok'></i></button>
								</form>
							</div>
							<?php }} else { if($assgn==''){} else{?>
