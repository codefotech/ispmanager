...
try {
    ...
    $mail->Username   = 'backupdb@asthatec.net';
    $mail->Password   = 'HkOQvffYE]ET';
    $mail->Port       = 25;
	$mail->SMTPSecure = 'tls';
	$mail->SMTPAuth = true;
    $mail->setFrom('backupdb@asthatec.net', 'TIS || Backup');

	foreach(array_unique($matches[0]) as $email) {
		$mail->AddAddress("$email", "$CompanyOwner ($CompanyName)");
	}
    $mail->addReplyTo('sabbironlyone@gmail.com', 'SaBBiR (Astha Technology)');

    $mail->addAttachment($file);         

	$mail->isHTML(true);                                  
    $mail->Subject = ''.$CompanyName.' || TIS || DB Backup  || '.$date_time.'';
	...
	$mail->Body = "<html>
					...
					<p>
						...
						<span style='font-family: georgia, palatino; font-size: large;'>Please <strong><span style='color: #ff0000;'>Keep it Safe</span></strong> for future inquiry.</span>
							<br />
							<br />
						<span style='font-family: georgia, palatino;'>[Backup by <strong><span style='color: #ff0000;'>".$user_name." (".$empid.")</span></strong>]</span>
					</p>
					<br /><br />
	...
	?>
<br />
	<div class="span6" style="width: 90%;/*! align-content: center; */text-align: center;">
	<h4 class="widgettitle">Full Database Backup</h4>
        <div class="widgetcontent">
			<div class="alert alert-success">
                <button data-dismiss="alert" class="close" type="button">&times;</button>
                <strong>Well done!!</strong> Database Backed Up Successfully!
            </div>
			<div class="alert alert-info">
                <button data-dismiss="alert" class="close" type="button">&times;</button>
                <strong>IMPORTANT NOTE</strong><br />Generating e-mail with full database backup. It will take time depends on your database size. <br />You will receive e-mail in your [ <?php echo $OwnerEmail; ?> ]. <br />Please keep the backup safe.
            </div>
			<div class="modal-body">
				<p>
					<span class="field"><a class="btn btn-green" onclick="history.back()"><<- Go Back</a></span>
				</p>
			</div>
		</div>
	</div>
