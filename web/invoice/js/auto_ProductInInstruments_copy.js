...
/* 			if( $('#qtysts_'+id[1]).val() == '1' ){
				document.getElementById("quantity_"+id[1].val()).readOnly = false;
				document.getElementById("slno_"+id[1].val()).readOnly = false;
			}
			else{
				document.getElementById("quantity_"+id[1].val()).readOnly = false;
				document.getElementById("slno_"+id[1].val()).readOnly = true;
			} */
		calculateTotal();
		}
	});
...
$(document).on('change keyup','.rtyudsrt',function(){
	id_arr = $(this).attr('id');
	id = id_arr.split("_");
	...
	for (i=0;i<quantity;i++){
		duration = $('.edhgdfh_'+itemNo+'_'+(i+1)).val();

		if(duration){
        $.ajax({  
				type: 'POST',
                url: "test2",
                data:{duration:'.edhgdfhss_'+itemNo+'_'+(i+1)},
                success:function(data){
                    $('#edhgdfhss_'+itemNo+'_'+(i+1)).html(data);
                }
        });
		return false;
		}
...
//				input.name = "slno" + i + "[]";
				...
				input.id = "slno_" + id[1];
				input.placeholder = itemName + " S/L No: " + (i+1);
				input.autocomplete = "off";
//				input.onchange = function(){itemNo+"_"+(i+1)};
				input.className = "rtyudsrt edhgdfh_"+itemNo+"_"+(i+1);
				input.style = "width: 52%;padding: 0px 4px 0px 4px;margin-bottom: 1px;text-align: center;float: left;";
				container.appendChild(input);
				
				div = document.createElement("div");
				div.style = "width: 2%;float: left;";
				div.id = "edhgdfhss_"+itemNo+'_'+(i+1);
