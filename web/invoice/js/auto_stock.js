...
$(document).on('focus','.autocomplete_txt',function(){
	...
	if(type =='qtyy')autoTypeNo=2;
	
	$(this).autocomplete({
		source: function( request, response ) {
			$.ajax({
				url : 'invoice/ajax_stock.php',
				dataType: "json",
				method: 'post',
				data: {
				   name_startsWith: request.term,
...
/* 			if($('#qty_'+id[1]).val(names[2]) == '0'){
				$('#quantity_'+id[1]).val(0).prop('readonly', false);
				$('#slno_'+id[1]).prop('required', false);
			}else{
				$('#quantity_'+id[1]).val(1).prop('readonly', true);
				$('#slno_'+id[1]).prop('required', true);
			} */
			qtyyy = $('#qty_'+id[1]).val(names[2]);
			if( qtyyy!='0' ) {$('#quantity_'+id[1]).val(1).prop('readonly', true);} else{$('#quantity_'+id[1]).val(0);}
//			$('#qty_'+id[1]).val(names[2]);
			$('#uniteprice_'+id[1]).val(0.00);
...
$(document).on('change keyup blur','.changesNo',function(){
	...
	if( quantity!='' && price !='' ) $('#price_'+id[1]).val( (parseFloat(price)*parseFloat(quantity)).toFixed(2) );	
	calculateTotal();
});
