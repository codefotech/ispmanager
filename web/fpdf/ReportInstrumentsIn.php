...
if($p_id == 'all'){
	class PDF extends PDF_MySQL_Table
		...
		function Header()
		{
			...
			$this->Cell(15,6,'Date','LTB',0,'C',true);
			$this->Cell(20,6,'V.No','LTB',0,'C',true);
			$this->Cell(22,6,'Purchaser','LTBR',0,'C',true);
			$this->Cell(25,6,'Vendor','LTB',0,'C',true);
			$this->Cell(13,6,'Brand','LTB',0,'C',true);
			$this->Cell(20,6,'S/L No','LTB',0,'C',true);
			$this->Cell(25,6,'Item','LTB',0,'C',true);
			$this->Cell(11,6,'U.price','LTB',0,'C',true);
			$this->Cell(8,6,'Qty','LTB',0,'C',true);
			$this->Cell(11,6,'Price','LTB',0,'C',true);
			$this->Cell(36,6,'Note','LTBR',0,'C',true);
			$this->Ln();
		}
		function Footer()
				{
		...
		$pdf->AddPage();
		if($e_id == 'all'){
		$result = mysql_query("SELECT i.id, i.purchase_date, i.voucher_no, i.purchase_by, e.e_name AS purchaseby, i.vendor, v.v_name, i.p_sts, i.p_sl_no, i.p_id, p.pro_name, i.brand, i.quantity, i.unite_price, i.price, i.rimarks, i.entry_by, a.e_name AS entryby, i.entry_time, i.sts FROM store_in_instruments AS i
		...
		while ($row=mysql_fetch_array($result)) {
				...
				$pdf->Cell(15,5,$purchase_date,'LTB',0,'L',true);
				$pdf->Cell(20,5,$voucher_no,'LTB',0,'L',true);
				$pdf->Cell(22,5,$purchaseby,'LTBR',0,'L',true);
				$pdf->Cell(25,5,$v_name,'LTB',0,'L',true);
				$pdf->Cell(13,5,$brand,'LTBR',0,'L',true);
				$pdf->Cell(20,5,$p_sl_no,'LTB',0,'L',true);
				$pdf->Cell(25,5,$pro_name,'LTB',0,'L',true);
				$pdf->Cell(11,5,$unite_price,'LTB',0,'R',true);
				$pdf->Cell(8,5,$quantity,'LTB',0,'C',true);
				$pdf->Cell(11,5,number_format($price,0),'LTB',0,'R',true);
				$pdf->Cell(36,5,$rimarks,'LTBR',0,'L',true);
				$pdf->Ln();
			...
			$pdf->Cell(151,6,'Total','LTB',0,'C',true);
			$pdf->Cell(8,6,number_format($Totalquantity,0),'LTBR',0,'R',true);
			$pdf->Cell(11,6,number_format($TotalPayment,0),'LTBR',0,'R',true);
			$pdf->Cell(36,6,'','LTBR',0,'L',true);
