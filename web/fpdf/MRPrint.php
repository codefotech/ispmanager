...
if($rowgg['mac_user'] == '1'){
$sql = mysql_query("SELECT p.id, c.c_name, p.c_id, p.pay_date, p.pay_date_time, e.e_name, e.e_cont_per, p.pay_amount, p.bill_discount, (p.pay_amount + p.bill_discount) AS paymentamount FROM payment_mac_client AS p 
...
if($rowgg['mac_user'] == '1'){
				$results = mysql_query("SELECT t2.c_id, IFNULL(t2.dis, 0.00) AS dis, IFNULL(t2.bill, 0.00) AS bill, IFNULL(t3.bill_disc, 0.00) AS bill_disc, IFNULL(t3.pay, 0.00) AS pay FROM
										(
										SELECT b.c_id, sum(b.bill_amount) AS bill, b.discount AS dis FROM billing_mac_client AS b
