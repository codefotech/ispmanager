...
if($p_id == 'all'){
	class PDF extends PDF_MySQL_Table
		...
		function Header()
		{
			...
			$this->Cell(30,6,'Out by','LTB',0,'C',true);
			$this->Cell(30,6,'Use by','LTB',0,'C',true);
			$this->Cell(67,6,'Note','LTBR',0,'C',true);
			$this->Ln();
		}
		function Footer()
				{
		...
		$pdf->AddPage();
		if($e_id == 'all'){
		$result = mysql_query("SELECT o.id, o.p_id, p.pro_name, o.qty, o.out_date, e.e_name AS out_by, k.e_name AS receive_by, o.note, o.out_date_time FROM store_out_instruments AS o
		...
		while ($row=mysql_fetch_array($result)) {
				...
				$pdf->Cell(10,5,$qty,'LTBR',0,'C',true);
				$pdf->Cell(30,5,$out_by,'LTB',0,'L',true);
				$pdf->Cell(30,5,$receive_by,'LTB',0,'L',true);
				$pdf->Cell(67,5,$note,'LTBR',0,'L',true);
				$pdf->Ln();
