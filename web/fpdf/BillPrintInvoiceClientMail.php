...
$quesdd = mysql_query("SELECT id, invoice_id, DATE_FORMAT(invoice_date, '%M-%Y') AS invoicedate, invoice_date, DATE_FORMAT(due_deadline, '%d-%m-%Y') AS due_deadline, DATE_FORMAT(due_deadline, '%d') AS payment_deadline FROM billing_invoice WHERE MONTH(invoice_date) = MONTH('$f_date') AND YEAR(invoice_date) = YEAR('$f_date') AND c_id = 'sabbir001' AND sts = '0' ORDER BY id ASC LIMIT 1");
