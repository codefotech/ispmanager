...
$API->debug = false;
						...
						if ($API->connect($ServerIP, $Username, $Pass, $Port)) {
							if($restor_clients == 'Yes'){
								$sql442 = mysql_query("SELECT c.c_name, c.c_id, p.mk_profile, c.p_id, p.p_name, p.bandwith, c.address, c.cell, c.note, c.note_auto, c.con_sts, DATE_FORMAT(c.join_date, '%D %M %Y') AS join_date, c.ip, c.mac, l.log_sts, l.pw FROM clients AS c
							...
							while( $rows1w = mysql_fetch_assoc($sql442) ){
								...
								foreach($arrID as $x => $x_value) {
									...
									if($rows1['c_name'] != ''){
											...
											$fghgdh = $rows1['c_id'];
											$dfhdfh_mk_profile = $rows1['mk_profile'];
											...
											$passs = sha1($x_value['password']);
												...
												if($sts_sync == 'Yes'){
													...
													if($x_value['profile'] == 'Inactive' && $rows1['con_sts'] == 'Active'){
														$arrID = $API->comm("/ppp/secret/getall", array(".proplist"=> ".id","?name" => $aaaaa,));
																 $API->comm("/ppp/secret/set",array(".id" => $arrID[0][".id"],"disabled"  => "no", "profile" => $dfhdfh_mk_profile));
