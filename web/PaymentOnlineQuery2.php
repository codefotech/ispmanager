...
$wayyy = isset($_GET['wayyy']) ? $_GET['wayyy'] : '';
...
if(mysql_num_rows($access) > 0 || $payfrom == 'external'){
//---------- Permission -----------
...
<h2>Payment Processing....</h2>
     <form action="PaymentOnlineMacSuccess?gateway=<?php echo $gateway;?>&wayyy=<?php echo $wayyy;?>" method="post" name="ok">
	 <input type="hidden" value="<?php echo $reseller_id; ?>" name="pay_ent_by" />
				...
				<input type="hidden" name="z_name" value="<?php echo $row22['z_name'];?>"/>
...
$result111 = mysql_query("SELECT c_id, c_name, cell, address, con_sts, b_date, payment_deadline, breseller FROM clients WHERE c_id = '$client_id'");
$row1111 = mysql_fetch_array($result111);

$query222 =mysql_query("SELECT agent_id, count_commission, com_percent FROM clients WHERE c_id = '$client_id'");
$row22 = mysql_fetch_assoc($query222);
...
<h2>Payment Processing....</h2>
     <form action="PaymentOnlineSuccess?gateway=<?php echo $gateway;?>&wayyy=<?php echo $wayyy;?>" method="post" name="ok">
				<input type="hidden" value="<?php echo $client_id; ?>" name="pay_ent_by" />
				<input type="hidden" value="<?php echo date('Y-m-d', time());?>" name="pay_ent_date" />
				<input type="hidden" name="pay_amount" value="<?php echo $dewamount;?>" />
...
<h2>Payment Processing....</h2>
     <form action="PaymentOnlineMacSuccess?gateway=<?php echo $gateway;?>&wayyy=<?php echo $wayyy;?>" method="post" name="ok">
				<input type="hidden" value="<?php echo $reseller_id;?>" name="pay_ent_by" />
				<input type="hidden" value="<?php echo $reseller_id;?>" name="e_id" />
				<input type="hidden" value="<?php echo $e_name;?>" name="e_name" />
				<input type="hidden" value="<?php echo $res_z_id;?>" name="z_id" />
				<input type="hidden" name="cell" value="<?php echo $e_cont_per;?>" />
				<input type="hidden" value="<?php echo date('Y-m-d', time());?>" name="pay_ent_date" />
				<input type="hidden" name="pay_amount" value="<?php echo $dewamount;?>" />
				<input type="hidden" name="sentsms" value="Yes" />
				<?php if($agent_id != ''){?>
...
<?php
	...
	$dfgdfgh = 'PaymentOnline?gateway='.$gateway.'&sts=faild&dewamount='.$dewamount.'&wayyy=reseller';
    header("Location:$dfgdfgh");
	}
}
else{
...
<?php
	...
	if($payfrom == 'external'){
		$dfgdfgh = 'PaymentOnlineExternal?gateway='.$gateway.'&sts=faild';
	}
	else{
		$dfgdfgh = 'PaymentOnline?gateway='.$gateway.'&sts=faild';
	}
