...
$titel = "Reports Graph";
$ReportGraph = 'active';
$ReportCollectionSumGraph = 'activeted';
...
if($t_id == 'all'){
		$query = mysql_query("SELECT CONCAT(z.z_name,' - ',z.z_bn_name) AS item, SUM(p.pay_amount) AS tot FROM payment AS p 
								LEFT JOIN clients AS c ON c.c_id = p.c_id 
								LEFT JOIN zone AS z ON z.z_id = c.z_id
								WHERE p.pay_date BETWEEN '$t_date' AND '$f_date'
								GROUP BY c.z_id");
		$tname = 'Collection Summary in All Zone';
		
}
else{
	$query = mysql_query("SELECT CONCAT(k.p_name,' - ',k.bandwith) AS item, SUM(p.pay_amount) AS tot FROM payment AS p 
								LEFT JOIN clients AS c ON c.c_id = p.c_id 
								LEFT JOIN zone AS z ON z.z_id = c.z_id
								LEFT JOIN package AS k
								ON k.p_id = c.p_id
								WHERE c.z_id = '$t_id' AND p.pay_date BETWEEN '$t_date' AND '$f_date'
								GROUP BY k.bandwith ORDER BY k.p_id ASC");
	...
	$tname = "Collection Summary in ".$ros['z_name'];
	}
}
...
<!-- -------------------------------------------------------------------------------------------------- -->
	...
	<div class="pageheader">
        ...
        <div class="pagetitle">
            <h1>Collection Summary Graph</h1>
        </div>
    </div><!--pageheader-->
