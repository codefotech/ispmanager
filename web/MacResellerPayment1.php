...
$qyaaaa = mysql_query("SELECT GROUP_CONCAT(c_id SEPARATOR ',') AS c_id FROM clients WHERE z_id = '$zz_id'");
...
$queryaaaa = mysql_query("SELECT COUNT(id) AS activeclient FROM clients WHERE con_sts = 'Active' AND z_id = '$zz_id' AND sts = '0'");
$rowaaaa = mysql_fetch_assoc($queryaaaa);
$activeclient = $rowaaaa['activeclient'];

$queryiiiii = mysql_query("SELECT COUNT(id) AS inactiveclient FROM clients WHERE con_sts = 'Inactive' AND z_id = '$zz_id' AND sts = '0'");
$rowiii = mysql_fetch_assoc($queryiiiii);
$inactiveclient = $rowiii['inactiveclient'];

$queryiiiiidd = mysql_query("SELECT COUNT(id) AS deletedclients FROM clients WHERE con_sts = 'Inactive' AND z_id = '$zz_id' AND sts = '1'");
$rowiiid = mysql_fetch_assoc($queryiiiiidd);
$deletedclients = $rowiiid['deletedclients'];

$totalclients = $activeclient + $inactiveclient + $deletedclients;

$total_offline_count = ($activeclient + $inactiveclient) - $total_online_count;

$sqlsdfs = mysql_query("SELECT * FROM emp_info WHERE z_id = '$zz_id'");
$rowfsdf = mysql_fetch_assoc($sqlsdfs);
$eee_id = $rowfsdf['e_id'];
$reseller_mk_id = $rowfsdf['mk_id'];
$billing_type = $rowfsdf['billing_type'];
$auto_recharge = $rowfsdf['auto_recharge'];
if($auto_recharge == '1'){
	$msges = 'Enable';
	$msges_color = 'red';
}
...
$minimum_days = $rowfsdf['minimum_days'];
if($billing_type == 'Postpaid'){
	$billingtype_color = '#953b39';
}
else{
	$billingtype_color = '#00a65a';
}
if($minimum_days == ''){
...
$query222 =mysql_query("SELECT z.z_id, z.z_name, z.z_bn_name, z.e_id, z.thana, COUNT(c.c_id) AS totalclients, e.e_name, e.e_cont_per, m.id, m.Name, m.ServerIP, e.pre_address, e.agent_id, e.count_commission, e.com_percent FROM zone AS z
...
$mk_name = $row22['Name'];
$mk_ip = $row22['ServerIP'];
$mkkkk_id = $row22['id'];

if($row22['z_bn_name'] != ''){
	$z_bn_name = '('.$row22['z_bn_name'].')';
}

$sql1dd = mysql_query("SELECT * FROM package WHERE z_id = '$zz_id' AND status = '0'");
...
<?php } ?>
	...
	<div class="pageheader">
        ...
        <div class="pagetitle">
            <h1>Reseller Profile (<?php echo $row22['e_name'].'-'.$row22['z_name'];?>)</h1>
        </div>
    </div><!--pageheader-->					
					...
					<div class="row">
						<div style="width: 100%;">
							...
							<table class="table table-bordered table-invoice" style="width: 48%; float: right;margin-right: 15px;">
								...
								<tr>
									<td>Total Online</td>
									<td><?php echo $total_online_count;?></td>
								</tr>
								<tr>
									<td>Total Offline</td>
									<td><?php echo $total_offline_count; ?></td>
								</tr>
							</table>
						</div><!--col-md-6-->
					</div>
		...
		<div style="width: 100%;">
			<div style="width: 100%;">
				...
				<div class="tabbedwidget">
                            <ul>
                                <li><b><a href="#tabs-1" <?php echo $color;?>>Bills & Payments</a></b></li>
                                <li><b><a href="#tabs-2">Network & Packages</a></b></li>
                                <li><b><a href="#tabs-3">Billing & Days</a></b></li>
                                <li><b><a href="#tabs-4">Clients</a></b></li>
                            </ul>
			...
			<div id="tabs-2">
						<p>
							<label style="font-weight: bold;">Mikrotik/Network</label>
							<span class="field">
								<a style="color: #317eac;font-size: 18px;font-weight: bold;line-height: 30px;"><?php echo $mk_name.' ('.$mk_ip.')';?></a>
							</span>
						</p>
						<p>
							<label style="font-weight: bold;">Zone/Area</label>
							<span class="field">
								<a style="color: #555;font-size: 18px;font-weight: bold;line-height: 30px;"><?php echo $row22['z_name'].' '.$z_bn_name;?></a>
							</span>
						</p>
						<p>
							<label style="font-weight: bold;"></label>
							<span class="field">
								<a href='<?php if($userr_typ == 'mreseller'){echo 'Clients';} else{echo 'Clients?id=macclient&zid='.$zz_id;}?>' title='Total Clients' class='label' style="font-size: 20px;font-weight: bold;padding: 3px 6px;border-radius: 3px;line-height: 21px;"><?php echo $totalclients;?></a>
								<a href='<?php if($userr_typ == 'mreseller'){echo 'Clients?id=active';} else{echo 'Clients?id=macclient&zid='.$zz_id;}?>' title='Total Active Clients' class='label label-success' style="font-size: 20px;font-weight: bold;padding: 3px 6px;border-radius: 3px;line-height: 21px;"><?php echo $activeclient;?></a> 
								<a href='<?php if($userr_typ == 'mreseller'){echo 'Clients?id=inactive';} else{echo 'Clients?id=macclient&zid='.$zz_id;}?>' title='Total Inactive Clients' class='label label-important' style="font-size: 20px;font-weight: bold;padding: 3px 6px;border-radius: 3px;line-height: 21px;"><?php echo $inactiveclient;?></a>
								<a style="font-size: 20px;font-weight: bold;color: #317eac;margin-left: 3px;"> & </a>
								<a href='Clients?id=left' title='Total Deleted Clients' class='label' style="background: #ff0000ba;font-size: 20px;font-weight: bold;padding: 3px 6px;border-radius: 3px;line-height: 21px;"><?php echo $deletedclients;?></a>
							</span>
						</p>
						<?php if($row22['thana'] != ''){ ?>
						<p>
							<label style="font-weight: bold;">Thana</label>
							<span class="field">
								<a style="color: #555;font-size: 18px;font-weight: bold;line-height: 30px;"><?php echo $row22['thana'];?></a>
							</span>
						</p>
						<?php } ?>
						<p>
							...
							<span class="field">
								<table class="table table-bordered responsive" style="text-align: center;width: 80%;">
									 <thead>
										<tr>
										<?php if($userr_typ == 'mreseller'){ ?>
											...
											<th style="width: 15%;padding:5px 0px 5px 5px; border-top: 2px solid #ddd;background: transparent; font-size: 14px;font-weight: normal;color: #ff7800;border-left: 0px solid;border-bottom: 3px solid #ddd;text-align: center;">Company PRICE</th>
											<th style="width: 15%;padding:5px 0px 5px 5px; border-top: 2px solid #ddd;background: transparent; font-size: 14px;font-weight: normal;color: #ff7800;border-left: 0px solid;border-bottom: 3px solid #ddd;text-align: center;">Own PRICE</th>
										<?php } else{ ?>
											<th style="width: 2%;padding:5px 0px 5px 5px; border-top: 2px solid #ddd;background: transparent; font-size: 14px;font-weight: normal;color: #ff7800;border-left: 0px solid;border-bottom: 3px solid #ddd;text-align: center;">ID</th>
									...
									<?php
										for($i=0; $i<20; $i++){											
											while($row2aa = mysql_fetch_assoc($sql1dd)){	$i = $i+1;			
												if($userr_typ == 'mreseller'){
													
													echo														
													"<tr>											
														<td class='center' style='font-size: 15px;font-weight: bold;padding-top: 12px;'>{$i}</td>											
														<td class='center'>													
															<input type='text' readonly style='width: 95%;font-weight: bold;' value='{$row2aa['p_name']}' />
														</td>											
														<td class='center'>												
															<input type='text' readonly style='font-weight: bold;width: 95%;' value='{$row2aa['bandwith']}'/>
														</td>											
														<td class='center'>												
															<input type='text' style='width: 60%;font-weight: bold;' readonly  value='{$row2aa['p_price']}'/><input type='text' style='width:13%; color:#2e3e4e; font-weight: bold;font-size: 13px;border-right: 1px solid #bbb;border-top: 1px solid #bbb;border-bottom: 1px solid #bbb;border-left: 0px solid #bbb;' value='৳' readonly />
														</td>											
														<td class='center'>												
															<input type='text' style='width: 60%;font-weight: bold;' readonly  value='{$row2aa['p_price_reseller']}'/><input type='text' style='width:13%; color:#2e3e4e; font-weight: bold;font-size: 13px;border-right: 1px solid #bbb;border-top: 1px solid #bbb;border-bottom: 1px solid #bbb;border-left: 0px solid #bbb;' value='৳' readonly />
														</td>											
														</tr>\n";
												}
												else{
													echo														
													"<tr>											
														<td class='center' style='font-size: 15px;font-weight: bold;padding-top: 12px;'>{$row2aa['p_id']}</td>											
														<td class='center'>													
															<input type='text' class='input-small' readonly style='width: 90%;font-weight: bold;' value='{$row2aa['mk_profile']}' />
														</td>
														<td class='center'>													
															<input type='text' class='input-small' readonly style='width: 90%;font-weight: bold;' value='{$row2aa['p_name']}' />
														</td>												
														<td class='center'>												
															<input type='text' class='input-small' readonly style='font-weight: bold;' value='{$row2aa['bandwith']}'/>
														</td>											
														<td class='center'>												
															<input type='text' style='width: 55%;font-weight: bold;' readonly  value='{$row2aa['p_price']}'/><input type='text' style='width:13%; color:#2e3e4e; font-weight: bold;font-size: 13px;border-right: 1px solid #bbb;border-top: 1px solid #bbb;border-bottom: 1px solid #bbb;border-left: 0px solid #bbb;' value='৳' readonly />
														</td>											
														</tr>\n";
												}												
											}  
			...
			<div id="tabs-3">
						<p>
							...
							<span class="field">
								<a style="color: <?php echo $billingtype_color;?>;font-size: 18px;font-weight: bold;line-height: 30px;"><?php echo $billing_type;?></a>
							</span>
						</p>
						<?php if($billing_type == 'Prepaid'){ ?>
						<p>
							<label style="font-weight: bold;">Due Limit</label>
							<span class="field">
								<a style="color: red;font-size: 18px;font-weight: bold;line-height: 30px;"><?php echo $rowfsdf['over_due'];?></a>
							</span>
						</p>
						<?php } if($rowfsdf['minimum_sts'] == '1') {?>
						<p>
							<label style="font-weight: bold;">Minimum Recharge Day</label>
							<span class="field">
								<a style="font-size: 20px;font-weight: bold;line-height: 30px;"><?php echo $rowfsdf['minimum_day'];?> Day</a>
							</span>
						</p>
						<?php } else{ ?>
						<p>
							...
							<span class="field">
								<?php foreach ($minimum_array as $item) { 
												echo "<a class='label label-success' style='font-size: 20px;font-weight: bold;padding: 3px 6px;border-radius: 3px;line-height: 20px;margin-right: 1px;'>$item</a>";
											}?>
							</span>
						</p>
						<?php } ?>
						<p>
							...
							<span class="field">
								<a style="color: <?php echo $msges_color;?>;font-size: 20px;font-weight: bold;line-height: 30px;"><?php echo $msges;?></a>
							</span>
						</p>
			</div>	
			<div id="tabs-4">
						<p>
							...
							<span class="field">
								<a href='Clients' title='Total Clients' class='label' style='font-size: 20px;font-weight: bold;padding: 3px 6px;border-radius: 3px;line-height: 21px;'><?php echo $totalclients;?></a> 
							</span>
						</p>
						<p>
							<label style="font-weight: bold;">Active</label>
							<span class="field">
								<a href='Clients?id=active' title='Total Active Clients' class='label label-success' style='font-size: 20px;font-weight: bold;padding: 3px 6px;border-radius: 3px;line-height: 21px;'><?php echo $activeclient;?></a> 
							</span>
						</p>
						<p>
							<label style="font-weight: bold;">Inactive</label>
							<span class="field">
								<a href='Clients?id=inactive' title='Total Inactive Clients' class='label label-important' style='font-size: 20px;font-weight: bold;padding: 3px 6px;border-radius: 3px;line-height: 21px;'><?php echo $inactiveclient;?></a> 
							</span>
						</p>
						<p>
							<label style="font-weight: bold;">Deleted</label>
							<span class="field">
								<a href='Clients?id=left' title='Total Deleted Clients' class='label' style='background-color: #0866c6ad !important;font-size: 20px;font-weight: bold;padding: 3px 6px;border-radius: 3px;line-height: 21px;'><?php echo $deletedclients;?></a> 
							</span>
						</p>
			</div>	
			</div>	
