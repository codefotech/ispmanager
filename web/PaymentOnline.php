...
if($paygrant == 'ok'){
	if($wayyy == 'reseller'){
		...
		$resultg = mysql_query("SELECT c_id, c_name, cell, address, con_sts, b_date, payment_deadline, breseller, mac_user, customerMsisdn, agreementID FROM clients WHERE c_id = '$e_id'");
		...
		if($row['breseller'] == '2'){
			...
			if($row['mac_user'] == '0'){
				...
				$resss = mysql_query("SELECT c_id FROM payment_mac_client WHERE c_id = '$e_id'");
				$rowsss = mysql_fetch_array($resss);
				$c_idd = $rowsss['c_id'];
				if($c_idd == ''){
					$res = mysql_query("SELECT c_id, SUM(bill_amount) AS amt FROM billing_mac_client WHERE c_id = '$e_id'");
					$rows = mysql_fetch_array($res);
					$Dew = $rows['amt'];
				}
				else{
					$res = mysql_query("SELECT (l.amt - (IFNULL(t.pay,0) + IFNULL(t.dic,0))) AS Dew FROM
										(
											SELECT c_id, SUM(bill_amount) AS amt FROM billing_mac_client WHERE c_id = '$e_id'
										)l
										LEFT JOIN
										(
											SELECT c_id, SUM(pay_amount) AS pay, SUM(bill_discount) AS dic FROM payment_mac_client WHERE c_id = '$e_id'
										)t
										ON l.c_id = t.c_id");
...
<?php } else {} ?>
	...
	<div class="pageheader">
        ...
        <div class="pagetitle">
            <h1>Pay With <?php echo $gateway;?></h1>
        </div>
    </div><!--pageheader-->
	<?php if($error != '' && $gateway == 'SSLCommerz') {?>
...
<script type="text/javascript">
    $(document).ready(function () {
            ...
            bKash.init({
                ...
                executeRequestOnAuthorization: function () {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            data = JSON.parse(data);
                             if (data && data.paymentID != null) {
								window.location.href = "PaymentOnlineQuery.php?paymentID="+data.paymentID+"&trxID="+data.trxID+"&createTime="+data.createTime+"&updateTime="+data.updateTime+"&transactionStatus="+data.transactionStatus+"&intent="+data.intent+"&merchantInvoiceNumber="+data.merchantInvoiceNumber+"&amount="+data.amount+"&gateway=bKash&wayyy=reseller&dewamount="+<?php echo $Dew;?>;
                            } else {
								alert(data.errorMessage);
                                window.location.href = "PaymentOnline?gateway=bKash&error="+data.errorMessage;
                            }
                        },
                        error: function () {
							alert(data.errorMessage);
				...
				onClose: function(){
					window.location.href='PaymentOnline?gateway=bKash&wayyy=reseller&dewamount='+<?php echo $Dew;?>;  // Your redirect route when cancel payment
				}
            });
...
<script type="text/javascript">
    $(document).ready(function () {
            ...
            bKash.init({
				...
				onClose: function(){
					window.location.href='PaymentOnline?gateway=bKash';  // Your redirect route when cancel payment
				}
            });
//			$('#bKash_button').removeAttr('disabled');
        });
