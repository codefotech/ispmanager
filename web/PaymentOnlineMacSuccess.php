...
<html>
<body>
    <form action="MacResellerPayment" method="post" name="ok">
		<input type="hidden" name="sts" value="faild">
		<input type="hidden" name="online_pay_amount" value="<?php echo $pay_amount;?>">
...
if($gateway == 'bKashT' && in_array(6, $online_getway)){
	...
	$querydfghdgh = "INSERT INTO payment_online (reseller_id, sender_no, pay_mode, date_time, paymentID, paymentExecuteTime, trxID, transactionStatus, amount, pay_amount, currency, intent, merchantInvoiceNumber, agreementID, customerMsisdn, payerReference) VALUES ('$e_id', '$customerMsisdn', '$gateway', '$date_time', '$paymentID', '$paymentExecuteTime', '$trxidd', '$transactionStatus', '$amount', '$dgdgdfg', '$currency', '$intent', '$merchantInvoiceNumber', '$agreementID', '$customerMsisdn', '$payerReference')";
	$resultsdfgs = mysql_query($querydfghdgh) or die("inser_query failed: " . mysql_error() . "<br />");
...
if($gateway == 'SSLCommerz' && in_array(3, $online_getway)){
	...
	$querydfghdgh = "INSERT INTO payment_online (reseller_id, pay_mode, date_time, paymentID, createTime, updateTime, trxID, transactionStatus, amount, pay_amount, currency, intent, merchantInvoiceNumber, refundAmount, card_type, bank_gw, card_no, card_issuer, card_brand, ssl_amount, alldata)
...
<html>
<body>
    <form action="PaymentOnline" method="post" name="ok">
		<input type="hidden" name="sts" value="faild">
		<input type="hidden" name="online_pay_amount" value="<?php echo $pay_amount;?>">
		<input type="hidden" name="mode" value="<?php echo $gateway;?>" />
    </form>
