...
$fileNameParts = pathinfo($PageName);
$linkName = $fileNameParts['filename'];

if(isset($_COOKIE['rememberme'])){
	$daysdd = 30*24*60*60*1000;
	setcookie ("remembermep", $linkName, time() + $daysdd, '/', '', true, true, "SameSite=None");
	$rawcookies = urlencode($_COOKIE['rememberme']);
	$queddryss = ("update login_cookie set last_page = '$linkName' WHERE cookie = '$rawcookies'");
	$sqddssl = mysql_query($queddryss) or die("Error" . mysql_error());
}

$e_id = $_SESSION['SESS_EMP_ID'];
$userr_typ = $_SESSION['SESS_USER_TYPE'];

$cudate_time = date('Y-m-d H:i:s', time());
	$qudryss = ("update login set last_active = '$cudate_time' WHERE e_id = '$e_id'");
	$sdssl = mysql_query($qudryss) or die("Error" . mysql_error());
...
<title><?php echo $titel.' | '.$CompanyName;?> | TIS</title>
...
{    
 ...
 {  
  ...
  if(name.length > <?php if($search_way == 'c_id'){echo '2';} elseif($search_way == 'com_id'){echo '0';} elseif($search_way == 'c_name'){echo '2';} elseif($search_way == 'ip'){echo '8';} else{echo '2';}?>)
  {
   $("#searchresult").html('');
   $.ajax({
...
<script>
 $(document).ready(function() {
 	 $("#responsecontainer10").load("AutoActiveResellerClientsCount.php?z_id=<?php echo $macz_id;?>");
...
<?php } if($userr_typ == 'admin' || $userr_typ == 'superadmin' || $userr_typ == 'billing_manager' || $userr_typ == 'accounts' || $userr_typ == 'mreseller' || $userr_typ == 'support' || $userr_typ == 'ets' || $userr_typ == 'support_manager' || $userr_typ == 'billing'){ ?>
			<li class="right">
...
<?php  } else {} if($userr_typ == 'mreseller') { ?>
	<li class="right"><div id='responsecontainer10' style="float: right;font-size: 18px;font-weight: bold;margin-right: 5px;margin-left: 5px;padding: 4px 0 4px 0;"></div> </li>
<?php  } ?>
